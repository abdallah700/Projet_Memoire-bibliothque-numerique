...
# student views
class SBookListView(LoginRequiredMixin,ListView):
    model = Book
...
class AReservationListView(LoginRequiredMixin, ListView):
    model = Reservation
...
class ADeleteReservation(LoginRequiredMixin, DeleteView):
    model = Reservation
    template_name = 'dashboard/confirm_delete.html'  # Assurez-vous que votre template est correctement nommé
    success_url = reverse_lazy('alreservation')  # Redirige après suppression vers la liste des réservations

    def delete(self, request, *args, **kwargs):
        # Incrémentez le nombre d'exemplaires disponibles si nécessaire
